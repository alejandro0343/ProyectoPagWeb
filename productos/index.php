...
$sel = $con->query("SELECT `pk_productos`,`descripcion`,`descripcionsub`, `cantidad`, `nombre_producto`, `precio`, `img_producto`
    FROM productos
INNER JOIN categorias
 on `fk_categorias` = pk_categorias
 INNER JOIN subcategorias
 on `fk_subcategorias` = pk_subcategorias");


$categorias = $con->query("SELECT DISTINCT pk_categorias,descripcion FROM `categorias`");
$subcategorias = $con->query("SELECT DISTINCT pk_subcategorias,descripcionsub FROM `subcategorias`");
?>
        ...
        <div class="row">
            <div class="col  ">
              ...
              <select name="categorias" class="form-select  mb-3" aria-label="Default select example">
                        ...
                        <?php while($t = $categorias->fetch_assoc()){ ?>
                            <option   value="<?php echo $t['pk_categorias'] ?>">
                            <?php echo $t['descripcion'] ?>
                        </option>
                        <?php } ?> 
                    </select>

                     <select name="subcategorias" class="form-select  mb-3" aria-label="Default select example">
                        <option  selected>Subcategoria</option>
                        <?php while($t = $subcategorias->fetch_assoc()){ ?>
                            <option   value="<?php echo $t['pk_subcategorias'] ?>">
                            <?php echo $t['descripcionsub'] ?>
                        </option>
                        <?php } ?> 
                    </select>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php while($f = $sel->fetch_assoc()){ ?>
                                <tr>
                                    <td><?php echo $f['pk_productos'] ?></td>
                                    <td><?php echo $f['cantidad'] ?></td>
                                    <td><?php echo $f['descripcion'] ?></td>
                                    <td><?php echo $f['descripcionsub'] ?></td>
                                    <td><?php echo $f['nombre_producto'] ?></td>
                                    <td><?php echo "$".number_format($f['precio'],2) ?></td>
                                    <td><?php echo $f['img_producto'] ?></td>
                                    <td><a href="editar.php?id=<?php echo $f['pk_productos'] ?>" class="btn btn-warning" >editar</a></td>
                                    <td><a href="eliminar.php?id=<?php echo $f['pk_productos'] ?>" class="btn btn-danger" >eliminar</a></td>
                                </tr>
                            <?php } ?> 
                        </tbody>
                    </table>
